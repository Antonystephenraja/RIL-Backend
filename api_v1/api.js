...
export const validateToken = (req, res) => {
  ...
  export const insertAuth = async (req, res) => {
    ...
    try {
        ...
        return res.status(201).json({ message: "Token inserted successfully", data: newData });
    } catch (error) {
  ...
  export const Insert=async(req,res)=>{
    ...
    try {
        ...
        const isTokenValid = await AuthModel.findOne({ "token.value": token.split(" ")[1] });
        if (!isTokenValid) {
...
export const InsetLimit = async (req, res) => {
  try {
    ...
    if (existingData) {
      ...
      const result = await InsertLimit.updateOne({ Id: '0001' }, { $set: updateData });
        ...
        if (data.length > 0) {
          const activityStatus = timeDiffInMinutes > 5 ? "inactive" : "active";
            ...
            const processSensorData = (sensorData) => {
              const validData = sensorData.map(item => parseFloat(item)).filter(item => !isNaN(item));
